    ...
    <div className="min-h-screen bg-[#060F1A]">
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs bg-[#14304F] text-[#9FB3CC] px-2 py-0.5 rounded" style={oswald}>Guardian</span>
          <form action={signOut}>
      ...
      <main className="max-w-4xl mx-auto px-5 py-8 space-y-8">
        {players && players.length > 0 ? players.map(player => {
          ...
          return (
            <div key={player.id}>
              <div className="flex items-center gap-3 mb-4 flex-wrap">
                ...
                <h2 className="text-lg text-[#E8EDF5]" style={oswald}>{player.full_name}</h2>
                {player.age_group && (
                ...
                {player.position && (
                  <span className="text-[10px] border border-[#1C3A5C] text-[#9FB3CC] px-2 py-0.5 rounded capitalize" style={oswald}>
                    {player.position}
                  </span>
                )}
              </div>
              ...
              {playerClips.length === 0 ? (
                <div className="bg-[#0B1E36] border border-[#1C3A5C] rounded-xl p-8 text-center">
                  <p className="text-sm text-[#4A6880]">No clips yet — your player's coach will upload them.</p>
                </div>
              ) : (
                <div className="bg-[#0B1E36] border border-[#1C3A5C] rounded-xl overflow-hidden">
                  <div className="h-1 bg-[#C8102E]" />
                  <div className="divide-y divide-[#1C3A5C]">
          ...
          <div className="bg-[#0B1E36] border border-[#1C3A5C] rounded-xl p-12 text-center">
            <p className="text-sm text-[#4A6880]">No players linked to your account yet.</p>
          </div>
        )}
      </main>
    </div>
